...
const QuizPage = () => {
  ...
  // 如果有项目ID，使用该项目的题目，否则使用默认题目
  let questions: Question[] = []
  let currentProject: Project | null = null
  
  if (projectId) {
    const project = projects.find(p => p.id === projectId)
    if (project && project.quiz) {
  ...
  const [timeRemaining, setTimeRemaining] = useState<number>(300) // 5分钟
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <div className="bg-white rounded-xl shadow-lg p-8 mb-8 fade-in">
        ...
        <h1 className="text-3xl font-bold text-center text-primary mb-4">
          {currentProject ? `${currentProject.title} - 知识测试` : 'pandas 知识测试'}
        </h1>
        <div className="flex justify-between items-center text-sm text-gray-600">
          <span>共 {questions.length} 道题，限时 5 分钟</span>
